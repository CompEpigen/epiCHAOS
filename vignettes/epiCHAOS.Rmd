...
**Note:** EpiCHAOS is designed for application to single-cell epigenomics datasets in binarised form, and is not intended for application to scRNA-seq data. Nevertheless we provide a function for quantifying transcriptional heterogeneity from scRNA-seq data using a similar approach which has been previously described in the literature [REF] Since we have not rigorously validated the performance of this approach, we reccomend that it be used with caution. 
...
# compute transcriptional heterogenetiy
